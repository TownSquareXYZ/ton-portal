...
const MARKET_CACHE_KEY = "market:ton";
const CACHE_TTL = 24 * 60 * 60; // 24小时
const MARKET_LAST_SYNC_KEY = "market:ton:last_sync";

const COINGECKO_API_URL = "https://api.coingecko.com/api/v3/coins/the-open-network";

// 创建代理配置
function createProxyAgent(url: string) {
...
function createFetchOptions() {
  const agent = createProxyAgent(COINGECKO_API_URL);
...
async function syncMarketData(): Promise<unknown> {
  ...
  const data = await retryOperation(async () => {
    ...
    const response = await fetch(COINGECKO_API_URL, fetchOptions as RequestInit);
    if (!response.ok) {
